...
class EntityLeveler(private val api: AuraSkillsApi, private val entityLeveler: SourceType) : Listener {
    ...
    fun onEntityDeath(e: EntityDeathEvent) {
        ...
        val damageSource = entity.lastDamageCause?.damageSource
        val direct = damageSource?.directEntity
        val causer = damageSource?.causingEntity
        val killer = causer as? Player ?: return
        ...
        for (skillSource in sources) {
            ...
            var skill = skillSource.skill()

            // Skip if the entity type doesn't match this source
            if (entity.type != source.entityType) continue

            // --- DAMAGER FILTER ---
            when (source.damager?.lowercase()) {

                "player" -> {
                    if (direct !is Player) continue
                    skill = Skills.FIGHTING
                }

                "projectile" -> {
                    if (direct !is Projectile) continue
                    if (causer !is Player) continue
            ...
            // Safety checks
            val context = LevelerContext(api, entityLeveler)
            if (context.failsChecks(killer, entity.location, skill)) continue
            ...
            api.getUser(killer.uniqueId)?.addSkillXp(skill, xp)
        }
    }
